...
import { VirtualizedFileList, ScrollIndicator } from './FileList.js';
...
export const FileNavigation: React.FC<FileNavigationProps> = ({
  ...
  // ファイルナビゲーションフック
  const navigation = useFileNavigation(initialPath) as UseFileNavigationExtended;
  ...
  const scrollInfo: ScrollInfo = {
    visibleStartIndex,
    visibleEndIndex: Math.min(visibleStartIndex + calculateAvailableHeight(), state.files.length),
  ...
  }, [state.selectedIndex, visibleStartIndex]);
    ...
    <Box flexDirection="column">
      ...
      {showDebug && (
        <Box marginBottom={1} borderStyle="single" padding={1}>
          <Text dimColor>
            Terminal: {terminalSize.height}x{terminalSize.width} | 
            Available: {calculateAvailableHeight()} | 
            Files: {state.files.length} | 
            Visible: {visibleStartIndex}-{Math.min(visibleStartIndex + calculateAvailableHeight(), state.files.length)} | 
 ...
 * ファイルナビゲーションコンテキスト
 */
const FileNavigationContext = React.createContext<UseFileNavigationExtended | null>(null);
...
export const useFileNavigationContext = () => {
  ...
  if (!context) {
    throw new Error('useFileNavigationContext must be used within FileNavigationProvider');
  }
  return context;
};
 ...
 */
export { useFileNavigation, useFileNavigationKeys } from '../hooks/useFileNavigation.js';
