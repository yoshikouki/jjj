...
export class PerformanceMonitor {
		...
		const sum = this.metrics.reduce(
			(acc, metrics) => ({
				...
				virtualScrollEfficiency:
					acc.virtualScrollEfficiency + metrics.virtualScrollEfficiency,
	...
	private calculateMin(): PerformanceMetrics {
		return this.metrics.reduce(
			(min, metrics) => ({
				...
				virtualScrollEfficiency: Math.min(
					min.virtualScrollEfficiency,
					metrics.virtualScrollEfficiency,
				),
	...
	private calculateMax(): PerformanceMetrics {
		return this.metrics.reduce(
			(max, metrics) => ({
				...
				virtualScrollEfficiency: Math.max(
					max.virtualScrollEfficiency,
					metrics.virtualScrollEfficiency,
				),
	...
	private calculateStandardDeviation(
		...
		const variance = this.metrics.reduce(
			(acc, metrics) => ({
				renderTime:
					acc.renderTime + (metrics.renderTime - average.renderTime) ** 2,
				memoryUsage:
					acc.memoryUsage + (metrics.memoryUsage - average.memoryUsage) ** 2,
				fps: acc.fps + (metrics.fps - average.fps) ** 2,
				virtualScrollEfficiency:
					acc.virtualScrollEfficiency +
					(metrics.virtualScrollEfficiency - average.virtualScrollEfficiency) **
						2,
				cacheHitRate:
					acc.cacheHitRate + (metrics.cacheHitRate - average.cacheHitRate) ** 2,
				fileLoadTime:
					acc.fileLoadTime + (metrics.fileLoadTime - average.fileLoadTime) ** 2,
				renderCount:
					acc.renderCount + (metrics.renderCount - average.renderCount) ** 2,
			}),
			this.getEmptyMetrics(),
		);
		...
		return {
			...
			virtualScrollEfficiency: Math.sqrt(
				variance.virtualScrollEfficiency / count,
			),
 ...
 */
export const useRenderTimeMeasure = (
	...
	useEffect(() => {
		if (enabled && startTimeRef.current > 0) {
			...
			if (renderTime > 16) {
				// 16ms以上の場合は警告
				console.warn(
					`${componentName} render time exceeded 16ms: ${renderTime.toFixed(2)}ms`,
				);
			}
		}
	});
...
/**
 * リアルタイムパフォーマンス表示（コンソール出力）
 */
export const logPerformanceStats = (stats: PerformanceStats): void => {
	console.log("=== Performance Stats ===");
	console.log(`FPS: ${stats.average.fps.toFixed(1)}`);
	console.log(`Render: ${stats.average.renderTime.toFixed(2)}ms`);
	console.log(
		`Memory: ${(stats.average.memoryUsage / 1024 / 1024).toFixed(2)}MB`,
	);
	console.log(
		`Cache Hit Rate: ${(stats.average.cacheHitRate * 100).toFixed(1)}%`,
	);
...
/**
 * パフォーマンス監視クラス（コンソール出力対応）
 */
export class ConsolePerformanceDisplay {
...
export class PerformanceWarningSystem {
	...
	checkMetrics(metrics: PerformanceMetrics): void {
		if (metrics.renderTime > this.thresholds.renderTime) {
			this.triggerWarning(
				`High render time: ${metrics.renderTime.toFixed(2)}ms`,
			);
		}
